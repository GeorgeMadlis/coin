# Research Protocol — {{TOPIC_TITLE}}

**Slug**: {{topic-slug}}  
**Started**: {{YYYY-MM-DD}}  
**Status**: active  
**Source**: {{URL or file path of raw material}}
...
3. For each work found that is relevant to {{TOPIC_TITLE}}, record:
...
```
Execute Stage 1 of research/{{topic-slug}}/PROTOCOL.md
Execute Stage 2 of research/{{topic-slug}}/PROTOCOL.md
Execute Stage 3 of research/{{topic-slug}}/PROTOCOL.md
Execute Stage 4 of research/{{topic-slug}}/PROTOCOL.md
```

After each stage, commit the outputs before proceeding:
`git add research/{{topic-slug}}/ && git commit -m "stage-N: {{topic-slug}}"`
